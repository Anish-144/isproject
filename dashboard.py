...
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="section-header">🌐 Attacker Geolocation Map</div>', unsafe_allow_html=True)

SEVERITY_COLORS = {"critical": "red", "high": "orange", "medium": "blue", "low": "green"}

# Build geo data from attack events
...
# Aggregate by IP + country for attempt count
ip_counts = {}
for r in geo_rows:
    key = r["ip"]
    if key not in ip_counts:
        ip_counts[key] = 0
    ip_counts[key] += 1
...
# Row 1: Attack Type Donut + MITRE Bar
...
# Row 3: Top Attacker IPs + Top Countries
...
with r3c2:
    # Build country stats from geo data
    if geo_rows:
...
# ═══════════════════════════════════════════════════════════════════════════════
# DETAILED EVENT LOG — Enhanced Table
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="section-header">🔍 Security Events</div>', unsafe_allow_html=True)

labels = {0: "benign", 1: "sqli", 2: "xss"}

tab1, tab2, tab3, tab4, tab5 = st.tabs(["📋 All Events", "🚨 Attacks", "🪤 Traps", "📁 Uploads", "🚫 Blocked IPs"])
...
with tab2:
    ...
    if attack_view.empty:
        ...
        for idx, row in attack_view.sort_values("time", ascending=False).head(50).iterrows():
            ...
            st.error(f"{icon} **{row.get('stored_class', '').upper()}**{mitre_tag} — {row.get('ip', '?')} — {row.get('method', '?')} {row.get('path', '?')} — {format_ts(row.get('time', ''))}")
...
with tab4:
    ...
    if upload_view.empty:
        ...
        for idx, row in upload_view.sort_values("time", ascending=False).head(50).iterrows():
            ...
            st.write(f"{uicon} **{details.get('filename', '?')}** ({details.get('file_size', 0)} bytes) — {details.get('user', '?')} — {format_ts(row.get('time', ''))}")
